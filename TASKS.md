...
**Проблема:**
Сейчас сводка "Кто что выбрал" показывает только общую сумму на человека и названия товаров без цен:
```
S8K: 4 поз. · 832.00 RUB
- Филе грудки индейки сыровяленое: 1 шт
- Напиток "Лесные ягоды": 1 шт
```

Непонятно сколько стоит каждый товар. И нет общего итога по всему заказу.

**Do:**
1. В `src/bot.py` функция `_format_who_chose_text()` (или аналогичная, которая генерирует сводку batch):
   - Рядом с каждым товаром добавить его цену: `- Филе грудки индейки: 1 шт · 250 ₽`
   - Цену брать из `discount_price` (если есть) или `price` из данных товара в state.db / batch_votes

2. В конце сводки добавить строку итого:
   ```
   Итого: 12 поз. · 2,601 ₽
   ```
   Где 12 = сумма всех позиций всех участников, 2601 = сумма всех цен.

**Files:** `src/bot.py`

**Done when:**
- В сводке batch рядом с каждым товаром видна цена
- В конце сводки есть строка "Итого: N поз. · X ₽"
- Если цена товара неизвестна (null) -- не показывать цену, но и не ломать сводку

**Do NOT:** не менять формат имени пользователя и количества позиций на человека -- только добавить цены
...
**Проблема:**
Карточки товаров в Mini App огромные: большая картинка, много пустого пространства, кнопка "Выбрать" на всю ширину. На экране помещается 1-2 товара. У ВкусВилла в приложении на том же экране видно 5-6 товаров.

**Референс (приложение ВкусВилл):**
- Горизонтальная строка: маленькая картинка слева (60x60) | название + наличие + цена | кнопка корзины справа
- Цена со скидкой крупно, старая цена зачёркнута мелко
- "В наличии N шт" мелким серым текстом
- Вся карточка = одна строка высотой ~70px

**Do в `webapp/index.html`:**
1. Переделать карточку товара из вертикальной (картинка сверху, текст снизу, кнопка внизу) в горизонтальную строку:
   - Слева: картинка 56x56px, border-radius: 8px
   - Центр: название (1-2 строки, overflow ellipsis), под ним серым мелким "В наличии N шт" или "Остаток уточняется"
   - Справа: цена со скидкой крупно (зелёный), старая цена зачёркнута мелко (серый), кнопка выбора (чекбокс или иконка корзины)
2. Высота карточки: ~70-80px максимум
3. Кнопка "Выбрать" заменить на компактный чекбокс или иконку корзины (как у ВкусВилла)
4. Выбранный товар: зелёная рамка или зелёный фон чекбокса (сейчас "Выбрано" кнопка -- слишком громоздко)
5. Убрать лишние отступы между карточками -- gap: 4-8px вместо текущего

**Files:** `webapp/index.html`

**Done when:**
- На экране 360x640 видно минимум 5 товаров без скролла (после шапки)
- Карточка содержит: картинку, название, наличие, цену, кнопку выбора
- Выбранный товар визуально отличается (зелёная рамка/чекбокс)
