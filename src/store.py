...
class StateStore:
        ...
        with self._connect() as conn:
            ...
            conn.execute(
                ...
                CREATE TABLE IF NOT EXISTS user_ledger (
                    ...
                    uniq_key TEXT NOT NULL DEFAULT '',
            ...
            conn.execute(
                """
                CREATE UNIQUE INDEX IF NOT EXISTS ux_user_ledger_uniq_key
                ON user_ledger(uniq_key)
                WHERE uniq_key <> ''
                """
            )
            ...
            if "availability_status" not in cols:
                conn.execute("ALTER TABLE items ADD COLUMN availability_status TEXT NOT NULL DEFAULT 'unknown'")
            if "availability_reason" not in cols:
                conn.execute("ALTER TABLE items ADD COLUMN availability_reason TEXT NOT NULL DEFAULT ''")
    ...
    @staticmethod
    def scoped_meta_key(scope: str | int | None, key: str) -> str:
        if scope in (None, ""):
            return key
        return f"{scope}:{key}"

    def set_scoped_meta(self, scope: str | int | None, key: str, value: str) -> None:
        self.set_meta(self.scoped_meta_key(scope, key), value)

    def get_scoped_meta(self, scope: str | int | None, key: str) -> str | None:
        return self.get_meta(self.scoped_meta_key(scope, key))

    def upsert_nutrition_cache(self, rows: list[NutritionCacheRow]) -> None:
        if not rows:
            return
        with self._connect() as conn:
            conn.executemany(
                ...
                INSERT INTO nutrition_cache(
                    item_id, url, kkal, protein_g, fat_g, carb_g, raw_text, status, error, updated_at
                ) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ...
    def get_nutrition_cache(self, item_id: str) -> NutritionCacheRow | None:
        with self._connect() as conn:
            row = conn.execute("SELECT * FROM nutrition_cache WHERE item_id = ?", (item_id,)).fetchone()
        if not row:
            return None
        return NutritionCacheRow(
            item_id=str(row["item_id"]),
            url=str(row["url"]),
            kkal=int(row["kkal"]) if row["kkal"] is not None else None,
            ...
            carb_g=float(row["carb_g"]) if row["carb_g"] is not None else None,
    ...
    def nutrition_cache_for_items(self, item_ids: list[str]) -> dict[str, NutritionCacheRow]:
        if not item_ids:
            return {}
        placeholders = ",".join("?" for _ in item_ids)
        with self._connect() as conn:
            rows = conn.execute(
                f"SELECT * FROM nutrition_cache WHERE item_id IN ({placeholders})",
                tuple(item_ids),
            ).fetchall()
        return {
            str(row["item_id"]): NutritionCacheRow(
                item_id=str(row["item_id"]),
                url=str(row["url"]),
                kkal=int(row["kkal"]) if row["kkal"] is not None else None,
                protein_g=float(row["protein_g"]) if row["protein_g"] is not None else None,
        ...
        with self._connect() as conn:
            for item in items:
                conn.execute(
                    """
                    INSERT INTO items(
                        day, item_id, name, price, discount_price, source, image_url, stock_qty,
    ...
    def sync_items(self, day: str, items: list[ItemRow], *, allow_delete: bool = False) -> tuple[list[ItemRow], int]:
        existing_ids = {row.item_id for row in self.list_items(day)}
        fresh: list[ItemRow] = []
        ...
        with self._connect() as conn:
            ...
            if allow_delete and new_ids:
                placeholders = ",".join("?" for _ in new_ids)
                # Protect items that have votes — don't delete them even if
                # they disappeared from the website (e.g. ready_food rotation).
                conn.execute(
                    f"DELETE FROM items WHERE day = ? AND item_id NOT IN ({placeholders})"
                    f" AND item_id NOT IN (SELECT DISTINCT item_id FROM batch_votes WHERE day = ?)",
                    [day, *new_ids, day],
                )
                # Only delete orphan votes (no batch_votes reference)
                conn.execute(
        ...
        removed = len(existing_ids - set(new_ids)) if allow_delete else 0
    ...
    def list_items(self, day: str) -> list[ItemRow]:
        with self._connect() as conn:
            rows = conn.execute(
                """
                SELECT item_id, name, price, discount_price, source, image_url, stock_qty,
                       availability_status, availability_reason
                FROM items
                WHERE day = ?
                ORDER BY rowid
    ...
    def votes_by_user(self, day: str, batch_id: int | None = None) -> list[dict]:
        ...
        with self._connect() as conn:
            rows = conn.execute(
                """
                SELECT user_id, user_name, item_id, qty, updated_at
                FROM batch_votes
                WHERE day = ? AND batch_id = ?
                ORDER BY updated_at, user_id, item_id
                """,
                (day, target.batch_id),
            ).fetchall()
        return [dict(row) for row in rows]

    def totals_by_user(self, day: str, batch_id: int | None = None) -> list[dict]:
        self.migrate_legacy_votes(day)
        target = self.get_cycle(day, batch_id) if batch_id is not None else self.get_open_cycle(day)
        if target is None:
            return []
        with self._connect() as conn:
            rows = conn.execute(
                ...
                SELECT
                    v.user_id,
                    v.user_name,
                    COUNT(CASE WHEN v.qty > 0 THEN 1 END) AS positions,
                    IFNULL(SUM(v.qty), 0) AS qty_total,
                    ROUND(IFNULL(SUM(i.discount_price * v.qty), 0), 2) AS total_sum
                FROM batch_votes v
                JOIN items i
                  ON i.day = v.day AND i.item_id = v.item_id
                WHERE v.day = ? AND v.batch_id = ? AND v.qty > 0
                GROUP BY v.user_id, v.user_name
                ORDER BY total_sum DESC, v.user_name
                """,
                (day, target.batch_id),
            ).fetchall()
        return [dict(row) for row in rows]
    ...
    def clear_day(self, day: str) -> None:
        with self._connect() as conn:
            ...
            conn.execute("DELETE FROM votes WHERE day = ?", (day,))
            conn.execute("DELETE FROM items WHERE day = ?", (day,))
            conn.execute("DELETE FROM user_ledger WHERE ref_day = ?", (day,))
            prefixes = (
                "last_collect_",
                "last_sessioncheck_",
                "last_mirror_",
                ...
                "last_finalize_",
                "last_executor_",
                "last_snapshot_restore_",
                "last_day_snapshot_",
                "best_day_snapshot_",
                f"excluded_item_ids:{day}",
            )
            for prefix in prefixes:
                if prefix.endswith(day):
                    conn.execute("DELETE FROM meta WHERE key = ?", (prefix,))
                else:
                    conn.execute("DELETE FROM meta WHERE key LIKE ?", (f"{prefix}%",))
    ...
    def get_day_snapshot(self, day: str, snapshot_id: str) -> DaySnapshot | None:
        with self._connect() as conn:
            row = conn.execute(
                """
                SELECT day, snapshot_id, created_at, regular_count, total_items, status, items_json
                FROM day_snapshots
                WHERE day = ? AND snapshot_id = ?
                LIMIT 1
                """,
                (day, snapshot_id),
            ).fetchone()
    ...
    def list_day_snapshots(self, day: str) -> list[DaySnapshot]:
        with self._connect() as conn:
            rows = conn.execute(
                """
                SELECT day, snapshot_id, created_at, regular_count, total_items, status, items_json
                ...
                if item_id:
                    conn.execute(
                        ...
                        SET added_qty = MIN(requested_qty, MAX(added_qty, ?)),
                            ...
                            last_error = ?,
                            updated_at = ?
                        WHERE day = ? AND batch_id = ? AND item_id = ?
                        """,
                        (
                            max(0, added_delta),
    ...
    def get_cycle_charge_entries(self, day: str, batch_id: int) -> list[LedgerEntry]:
        with self._connect() as conn:
            rows = conn.execute(
                """
                SELECT id, uniq_key, ref_day, batch_id, user_id, user_name, kind, amount, note, created_at
                FROM user_ledger
                WHERE ref_day = ? AND batch_id = ? AND kind = 'charge'
                ORDER BY user_name
                """,
                (day, batch_id),
            ).fetchall()
    ...
    def ensure_cycle_charges(self, day: str, batch_id: int, note: str = "") -> list[LedgerEntry]:
        rows = self.totals_by_user(day, batch_id=batch_id)
        now = self._now_iso()
        with self._connect() as conn:
            for row in rows:
                amount = round(float(row.get("total_sum") or 0), 2)
                if amount <= 0:
                    continue
                user_id = int(row["user_id"])
                uniq_key = f"charge:{day}:{batch_id}:{user_id}"
                conn.execute(
                    """
                    INSERT OR IGNORE INTO user_ledger(
                        uniq_key, ref_day, batch_id, user_id, user_name, kind, amount, note, created_at
                    )
                    VALUES(?, ?, ?, ?, ?, 'charge', ?, ?, ?)
                    """,
                    (
                        ...
                        int(batch_id),
                        user_id,
                        str(row.get("user_name") or f"user {user_id}"),
                        amount,
                        note,
                        now,
                    ),
                )
        return self.get_cycle_charge_entries(day, batch_id)
        ...
        with self._connect() as conn:
            rows = conn.execute(
                ...
                SELECT
                    ...
                    ROUND(SUM(CASE WHEN kind = 'charge' THEN amount ELSE -amount END), 2) AS balance,
                    ROUND(SUM(CASE WHEN kind = 'charge' THEN amount ELSE 0 END), 2) AS charges_total,
                    ROUND(SUM(CASE WHEN kind = 'payment' THEN amount ELSE 0 END), 2) AS payments_total,
                    MAX(created_at) AS last_activity_at
                FROM user_ledger
                GROUP BY user_id
                HAVING balance > 0.009
                ORDER BY balance DESC, user_name
                """
            ).fetchall()
        return [dict(row) for row in rows]

    def get_outstanding_balance(self, user_id: int) -> dict | None:
        with self._connect() as conn:
            row = conn.execute(
                """
                SELECT
    ...
    def register_payment(self, user_id: int, user_name: str, amount: float, note: str = "") -> float:
        ...
        with self._connect() as conn:
            conn.execute(
                ...
                VALUES('', '', 0, ?, ?, 'payment', ?, ?, ?)
                """,
                (int(user_id), str(user_name or f"user {user_id}"), paid, note, self._now_iso()),
            )
        return paid
